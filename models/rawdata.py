...
# +----------------------+--------------+------+-----+---------+-----------------+
# | rawdata_id           | biginteger   | NO   | PRI |         | auto_increment  |
# | sensor_data_date     | datetime     | NO   |     |         |                 |
# | sensor_name          | varchar(30)  | NO   |     |         |                 |
...
class RawdataModel(db.Model):
    ...
    rawdata_id =        db.Column(db.BigInteger, primary_key=True)                              # 원본 데이터 아이디
    sensor_data_date =  db.Column(db.DateTime, nullable=False)                               # 데이터 들어온 날짜
    sensor_name =         db.Column(db.String(30), nullable=False)                             # 센서 아이디
