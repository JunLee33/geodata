...
# +----------------------+--------------+------+-----+---------+-----------------+
# | function_id          | biginteger   | NO   | PRI |         | auto_increment  |
# | function_name        | varchar(20)  | NO   |     |         |                 |
# | function_formula     | varchar(200) | NO   |     |         |                 |
# | function_type        | varchar(4)   |      |     | "Y"     |                 |
# | use_yn               | varchar(1)   | NO   |     | "Y"     |                 |
# | user_id              | varchar(20)  | NO   |     |         |                 |
# | project_id           | integer      |      |     |         | tbl_project     |
# | company_id           | integer      |      |     |         | tbl_project     |
# | sensorgroup_id       | integer      |      |     |         | tbl_project     |
...
class FunctionModel(db.Model):
    ...
    company_id =        db.Column(db.Integer, nullable=True)                                   # 변위공식 적용되는 컴퍼니 아이디
    sensorgroup_id =    db.Column(db.Integer, nullable=True)                                   # 변위공식 적용되는 센서 그룹아이디 (로드셀)
    create_date =       db.Column(db.DateTime, nullable=False, default=datetime.now())
    modify_date =       db.Column(db.DateTime, nullable=False, default=datetime.now())

    def __init__(self, function_name,  function_formula, function_type, use_yn, user_id, project_id, company_id, sensorgroup_id, create_date, modify_date):
    ...
    def find_by_project_id(cls, project_id):
        sql = """select function_id, function_name, function_formula from tbl_function
                where use_yn = 'Y' and project_id = '"""+project_id+"""' and function_type = '0002'
                    order by create_date desc;"""
    ...
    @classmethod
    def find_by_roadcell(cls, project_id, sensorgroup_id):
        print("find_by_roadcell!!")
        sql = """select function_id, function_name, function_formula from tbl_function
                where use_yn = 'Y' and project_id = '"""+project_id+"""' and sensorgroup_id = '"""+ sensorgroup_id+"""' and function_type = '0204'
                    order by create_date desc;"""
