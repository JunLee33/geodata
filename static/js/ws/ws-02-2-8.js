...
function savebtn(){
    ...
    while (true) {
        let found_minus_x = editdatavalue_x.indexOf(search_minus, pos_minus_x);   // 1부터 검색했을때 있으면, 터짐
        if (found_minus_x == -1){
            break;
        }else {
            alert("X 데이터를 확인해주세요.");
    ...
    while (true) {
        let found_minus_y = editdatavalue_y.indexOf(search_minus, pos_minus_y);   // 1부터 검색했을때 있으면, 터짐
        if (found_minus_y == -1){
            break;
        }else {
            alert("Y 데이터를 확인해주세요.");
