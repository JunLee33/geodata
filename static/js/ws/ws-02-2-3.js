...
function uploadFile(input){
    ...
    let uploadconfirm = confirm("데이터 upload시 값이 수정됩니다. upload 하시겠습니까?")
...
function savebtn(){
    ...
    let cnt_dot = 0;
    while (true) {
        let foundPos = editdatavalue.indexOf(search_dot, pos);      // pos부터 find
        console.log(cnt_dot)
        console.log(foundPos)
        if (foundPos == -1){
            break;
        }else if(foundPos == 0 || foundPos == editdatavalue.length - 1){
            alert("데이터를 확인해주세요.");
            return;
        } 
    ...
    while (true) {
        let found_minus = editdatavalue.indexOf(search_minus, pos_minus);   // 1부터 검색했을때 있으면, 터짐
        if (found_minus == -1){
            break;
        }else {
            alert("데이터를 확인해주세요.");
