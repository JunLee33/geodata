    ...
    $.ajax({
        ...
        error:function(err){
         ...
         success:function(json) {
            ...
            $('#fomulaname').text(sensordata[0].function_name)
            ...
            $.ajax({
                url:"/function_formula/roadcell/select?function_id="+sensordata[0].sensorgroup_fx,
                type:"get",
                contentType: false,
                processData : false,
                error:function(err){
            ...
            setTimeout(() => {
                ...
                $.ajax({
                    ...
                    success:function(data) {
                        ...
                        // window.location.href =  "/ws-02-1-13?project_id="+project_id+"&sensorgroup_id="+sensorgroup_id+"&sensorgroup_type="+sensorgroup_type+"&date_time_start="+next_time_start+"&date_time_end="+next_time_end+"&intervalday="+intervalday+"&time="+time
...
function selectgraph(){
    ...
    const colors = [
        ...
        for(let i =0; i<graphdata.length; i++){
            ...
            data.push(graphdata[i].sensor_fx_data)
        
        }

        for(let i=0; i<sensor_display_name_list.length; i++){
            displayname += sensor_display_name_list[i]+" "
        }
