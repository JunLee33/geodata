...
$(function(){
    ...
    $('#savebtn').click(function(){
        ...
        console.log(sensorgroup_gauge_factor)

        if(sensorgroup_gauge_factor.length == 0){alert("값을 입력해 주세요"); $('#gaudefactorint').focus(); return;}
        ...
        let cnt_dot = 0;
        while (true) {
            let foundPos = sensorgroup_gauge_factor.indexOf(searchvalue, pos);
            if (foundPos == -1){
                break;
            }else if(foundPos == 0 || foundPos == sensorgroup_gauge_factor.length - 1){
                alert("값을 확인해주세요.");
                return;
            } 
            pos = foundPos + 1; 
