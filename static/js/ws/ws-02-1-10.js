...
function selecttable(){
    ...
    $('#tabletbody').append("<tr class='uk-text-success' id='Displacement'><td>최대변위</td></tr>")
    ...
    for(let i=0; i<sensorname_list_xy.length; i++){
        ...
        for(let j=0; j<datalist[0].length; j++){
            ...
            if(datalist[i][j]){
                let displacement_x =Math.abs(parseFloat(datalist[i][j].sensor_fx_check_data_x))
                let displacement_y = Math.abs(parseFloat(datalist[i][j].sensor_fx_check_data_y))
                if(j == 0){
                    data_X = displacement_x
                    current_fx_data_x = datalist[i][j].sensor_fx_check_data_x
                    data_Y = displacement_y
