$(function(){
    ...
    $.ajax({
        ...
        success:function(data) {
            ...
            for(let i = 0; i <companyProject.length; i++){
                ...
                if(projectList.indexOf(companyProject[i].project_id) == -1){
                    ...
                    if(current_end_day.length != 0 ){
                        projectList.push(companyProject[i].project_id)
                        let lastdate = moment(current_end_day, 'YYYY-MM-DDTHH:mm:ssZ');
                        console.log(lastdate)
                        var now = moment();
                        var oneMonthAgo = now.add(1,'months');
                        now = moment();
            ...
            console.log("현재 : ", now);
            var oneMonthAgo = now.add(1,'months');	// 한달 뒤
            console.log("한달 뒤 : ", oneMonthAgo);
        }
    })
})