...
class editdatatableall(Resource):
    ...
    def post(self):
        ...
        for i in range(len(sensordata_list)):
            ...
            data123 = [dict(r) for r in EditdataModel.editdata_sensor_name_dataroggerid(param)]

            if len(datelist) < len(data123):
                datelist = []
                for i in range(len(data123)):
                    datelist.append({"sensor_data_date":data123[i]['sensor_data_date']})
            ...
            if sensorlist:
                ...
                if  len(time) ==0 and len(intervalday) !=0:
                    ...
                    sensordatas = copy.deepcopy(data123)
...
class editdatatableallgraph(Resource):
    ...
    def post(self):
        ...
        for i in range(len(sensordata_list)):
            ...
            if len(datelist) < len(data123):
                for j in range(len(data123)):
                    datelist.append({"sensor_data_date":data123[j]['sensor_data_date']})
    ...
    def get(self):
        ...
        for i in range(len(sensordata_list)):
            if sensorgroup_type == '0205':
                ...
                current_inteval = float(decimal.Decimal(sensorgroup_interval)* decimal.Decimal(i+1))
...
class excelupload(Resource):
    ...
    def post(self):
        ...
        sensor_id =              params['sensor_id']
        print("IN excelupload! sensor_id : "+str(sensor_id))
        ...
        try:
            ...
            if sensor_id:
                for i in range(len(xlsx)):
                    ...
                    # print("EXCEL : date : " + str(sensor_data_date) + "/ data : " + str(sensor_data) + "/ idx : " + str(i))
...
class editdataroadcell(Resource):
    ...
    def post(self):
        ...
        fx_list = [dict(r) for r in FunctionModel.find_by_roadcell(str(project_id), str(sensorgroup_id))]
...
class editdatatableallline(Resource):
    ...
    def post(self):
        ...
        # 가로형
            for idx in range(len(totaldatalist[0])):
                ...
                for j in range(len(totaldatalist)):
                    ...
                    alldata[idx][sensorM[current_idx]] = str(round(totaldata, 4))
                    groupinterval.append(sensorM[current_idx])
        ...
        return {'resultCode': '0', "resultString": "SUCCESS", "data": totaldatalist, "table_fx_data": alldata, "groupinterval" :groupinterval}, 200
